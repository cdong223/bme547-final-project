...
def data_interface_window(username='NA'):
    ...
    def saved_images_listbox_execution():
        ...
        if not saved_images:
            ...
            cancel_saved_images_button = ttk.Button(saved_images_windows, text="Cancel",
                                                    command=cancel_saved_images_button_execution)
            cancel_saved_images_button.grid(column=4, row=5, columnspan=3)

            saved_images_var = StringVar(value=saved_images)
            saved_images_listbox = Listbox(saved_images_windows, selectmode='multiple', height=len(saved_images),
                                           listvariable=saved_images_var)
            saved_images_listbox.grid(column=2, row=1, columnspan=3)

    download_tab.title("Image Download")

    top_label = ttk.Label(download_tab, text="Image Download")
    top_label.grid(column=2, row=0, columnspan=3)

    image_type = StringVar()
    jpeg_option = ttk.Radiobutton(download_tab, text="JPEG", variable=image_type, value="JPEG")
    jpeg_option.grid(column=2, row=2)

    png_option = ttk.Radiobutton(download_tab, text="PNG", variable=image_type, value="PNG")
    png_option.grid(column=2, row=3)

    tiff_option = ttk.Radiobutton(download_tab, text="TIFF", variable=image_type, value="TIFF")
    tiff_option.grid(column=2, row=4)

    download_button = ttk.Button(download_tab, text="Download", command=download_button_execution)
    download_button.grid(column=0, row=5, columnspan=3)

    saved_images_button = ttk.Button(download_tab, text="Saved Images", command=saved_images_listbox_execution)
